    ...
    <div className="grid gap-3 rounded-lg border border-border bg-muted/40 p-4">
      ...
      {answer ? (
        <div className="grid gap-2">
          {answer.warning ? (
            <p className="rounded-md border border-amber-200 bg-amber-50 p-2 text-xs leading-5 text-amber-900">{answer.warning}</p>
          ) : null}
          <pre className="whitespace-pre-wrap rounded-md bg-background p-3 font-sans text-xs leading-6 text-foreground">
            {answer.answer}
          </pre>
        </div>
      ) : null}
